...
//
//   @override
//   State<StatefulWidget> createState() => DialogAppearanceModeState(
//       /*valueOne: valueOne,title: this.title, valueTwo: valueTwo,*/ /*functionOne: functionOne,functionTwo: functionTwo*/);
// }
//
...
//         AppCubit.get(context).isDark ? 'Night mode' : 'Light mode';
//     return Center(
//       child: Material(
...
//   }
//
//   Container buildDialogContainer({
...
class DialogChangeLanguageState extends State<DialogChangeLanguage>
  ...
  Container buildDialogContainer({
    ...
    String languageMode = CacheHelper.returnData(key: 'lang') == 'en' ? 'اللغة العربية' : 'English';
    return Container(
        margin: const EdgeInsets.all(20.0),
        padding: const EdgeInsets.all(15.0),
        height: 30.h,
                ...
                Padding(
                  ...
                  child: defaultButton(
                    function: () {
                      setState(() {
                        Navigator.pop(context);
                        // AppCubit.get(context).setLocale(const Locale('ar'));
                        CacheHelper.returnData(key: 'lang') == 'en'
                            ? AppCubit.get(context).setLocale(const Locale('ar'))
                            : AppCubit.get(context).setLocale(const Locale('en'));
                        print(
                            'Language mode value from cache is ${CacheHelper.returnData(key: 'lang')}');
                      });
                    },
                    textData: languageMode,
                    textSize: 12.sp,
                    width: 40.w,
                    height: 6.h,
                    isUpperCase: false,
